...
### Added
- **The release is one command, and it ends with the exe in `dist/`.**
  `scripts/release.sh` runs the whole recipe - build, package, build again,
  package again - and `scripts/verify-release.sh` runs at the end of packaging
  to fail when `dist/hgfloater.exe` is missing, empty, or carrying a version
  other than the one being released. The artifact is the built executable
  itself, plain; a zip of the same exe is an extra, made with
  `HG_RELEASE_ZIP=1` for a browser that objects to a bare unsigned `.exe`.
  Both READMEs state this in section 14.4. It was already the practice; it is
  a check now, because a practice is something that can be forgotten once.
