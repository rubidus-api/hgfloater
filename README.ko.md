...
**목록**은 한 줄에 노트 하나씩 생성일, 최종 수정일, 제목 순으로 보여 주고, 최근에 고친
것이 위로 옵니다. 제목 앞의 `*`는 보존 처리된 노트라는 표시입니다.

| 키 | 하는 일 |
| :--- | :--- |
| `Enter`, 더블 클릭 | 노트를 별개의 편집 창으로 엽니다 |
| `Insert` | 노트를 새로 만들고 바로 엽니다 |
| `K` | 선택한 노트의 보존 처리를 켜고 끕니다 |
| `Delete` | 선택한 노트를 지웁니다. 보존 처리된 노트는 거부하므로 `K`로 먼저 풀어야 합니다 |
| `Esc` | 목록을 닫습니다 |
...
텍스트 파일이 담을 수 없는 것은 보존 플래그 하나뿐이고, 그것만 옆에 있는
`note\index.ini`에 따로 둡니다.
...
| `%USERPROFILE%\.HellGates\hgfloater\note\` | [노트](#10-노트) 하나당 `.txt` 하나, 보존 플래그용 `index.ini` |
