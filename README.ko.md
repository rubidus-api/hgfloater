...
| `Shift + 위/아래` | 위 결과창 스크롤 |
| `Shift + PgUp/PgDn` | 위 결과창을 한 페이지씩 스크롤 |
| `Shift + 좌/우` | 히스토리의 이전 / 다음 줄 |
| `Ctrl + Space` | 입력칸으로 이동 |
| `Esc` | 창 닫기 |
...
Shift + 화살표는 결과창과 히스토리 몫이라 입력칸의 글자 선택에는 더 이상 쓰이지 않습니다 —
**`Ctrl + Shift + 좌/우`**는 여전히 단어 단위로 선택합니다.
