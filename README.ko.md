...
**v0.15.0** — 빌드 2026-08-14 23:10 KST
...
- **왼쪽 클릭** — 플로터 어디를 눌러도 태스크박스를 토글합니다. 그 자리에서 열리며,
  태스크박스가 떠 있는 동안 플로터는 스스로 숨습니다. 포인터가 지나가기만 해서는 아무 일도
  없고, **Open the Taskbox on Hover** 를 켠 경우에만, 그것도 시계 위에서만 반응합니다.
...
- **Options** — 켜고 끄는 항목은 모두 이 하위 메뉴에 모여 있습니다. 켜져 있으면 체크가
  ...
  - **Open the Taskbox on Hover** — 기본은 꺼짐입니다. 켜면 v0.13.0 이전처럼 포인터가
    **시계 위에** 머무는 것만으로 대시보드가 열리고, 꺼져 있으면 클릭으로 엽니다. 시계
    영역만 반응하는 이유는, 플로터의 나머지는 손이 다른 곳으로 가다 스치는 자리이기
    때문입니다. 어느 쪽이든 **클릭은 플로터 어디서나** 태스크박스를 엽니다.
    [4장](#4-플로터)을 보세요.
  ...
  - **Sensor Bars on the Floater** — 시계 뒤에 깔리는 배터리·CPU·메모리·온도 막대입니다.
