...
> ### 📌 단축 아이콘 등록하는 법
>
> 1. **옵션 메뉴를 엽니다** — `Set` 버튼의 `Opt` 줄, 또는 상태 줄 오른쪽 클릭. 옵션 메뉴가
>    열립니다.
...
| `왼쪽`/`오른쪽` | 박스에서 빠져나와 **왼쪽·오른쪽 아이콘으로** 이동합니다. 목록은 세로줄이고, 좌우는 그리드에서 원래 그런 뜻이기 때문입니다. 옮겨 간 아이콘에 목록이 있으면 그쪽 목록이 열립니다 |
...
- **오른쪽 클릭**하면 옵션 메뉴(`Set` 목록의 `Opt` 줄과 동일)가 열립니다.
...
내장 버튼 9개가 아이콘과 같은 격자에 놓입니다. 순서는 고정입니다. 글자 하나 대신 짧은
단어를 적으며, 그중 둘은 무언가를 바로 하는 대신 **목록을 엽니다.**
...
| **`Mv`** Move | **클릭**하면 스스로 비켜남(아래 참고) | **드래그**로 창 이동 |
| **`X`** Exit | HGFloater 종료 | — |
...
| **`Set`** Settings | [컨트롤 목록](#62-set-버튼)을 엽니다: 볼륨·밝기·투명도·고정·옵션 메뉴 | — |
...
목록이 열립니다. 가리키는 것과 키보드로 도착하는 것은 같은 도착이라, 어느 쪽도 클릭을 먼저
요구하지 않습니다.
...
| 줄 | 하는 일 |
| :--- | :--- |
| **Options Menu** | [옵션 메뉴](#7-옵션-메뉴)를 엽니다 — 포인터 자리가 아니라 목록이 있던 자리, 즉 **`Set` 버튼 바로 아래**에 뜹니다. 이 목록에서 다른 곳으로 나가는 유일한 줄이라 맨 위입니다 |
| **Volume (ScrollWheel)** | 시스템 볼륨과 현재 값. 줄 위에서 **휠**을 돌리면 5% 단위로 바뀌고, **클릭**하면 음소거를 토글합니다 |
| **Brightness (ScrollWheel)** | 화면 밝기와 현재 값. **휠**로 5% 단위 조절 |
| **Alpha (ScrollWheel)** | 태스크박스 투명도와 현재 값. **휠**로 조절 |
| **Pin** | 태스크박스 고정. 고정된 동안에는 마우스를 치워도 접히지 않습니다. **클릭**으로 토글 |
| **스위치들** | 켜고 끄는 설정 전부가 한 줄씩, 옆에 `on`/`off` 가 붙습니다. **클릭**으로 토글 |
...
**`Mv` — 비켜나기.** 드래그하지 않고 클릭하면, 방금 있던 영역을 딱 벗어날 만큼만 스스로
...
[`Set` 버튼](#62-set-버튼)의 `Opt` 줄, 또는 상태 표시줄 오른쪽 클릭으로 엽니다.
...
| **옵션 메뉴** | `Set` 목록의 `Opt` 줄, 또는 상태 표시줄 오른쪽 클릭 |
| **폴더 열기** | `Dir` 에 포인터를 두고 줄 클릭 |
| **컨트롤 목록** | `Set` 에 포인터 두기 |
| **창 이동** | 빈 공간·상태 표시줄·`Mv` 버튼 왼쪽 드래그 |
| **플로터 이동** | 플로터를 드래그, 또는 `Alt + 드래그` |
| **태스크박스 비켜나기** | `Mv` 버튼 왼쪽 클릭 |
