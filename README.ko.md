...
**브라우저와 탐색기의 탭도 각각 아이콘으로 띄울 수 있습니다.** 기본은 꺼짐이고, `O` 메뉴의
**Show Browser & Explorer Tabs** 또는 `config.ini` 의 `[taskbox] show_tabs=1` 로 켭니다. 켜면
탭이 여덟 개인 브라우저 창은 아이콘 여덟 개가 되고, 하나를 누르면 그 탭으로 전환됩니다.

기본이 꺼짐인 데는 알아 둘 만한 이유가 있습니다. 탭은 창이 아니라서 찾을 방법이
**UI Automation** 뿐인데, 이건 상대 프로그램의 UI 스레드로 들어가는 호출이라 느리고 그쪽이
바쁘면 멈춥니다. 그래서 탭이 있을 수 있는 창(크로미움 계열 브라우저·탐색기)에만 묻고, 1초가
아니라 5초마다 다시 읽으며, 실패하면 조용히 창 하나짜리 아이콘으로 되돌아갑니다. 화면 밖 탭을
아예 공개하지 않는 프로그램도 있는데 그런 탭은 나타나지 않습니다. 설계와 비용은
...
- **Show Browser & Explorer Tabs** — 탭을 각각의 작업 아이콘으로 띄울 때 체크됩니다.
  비용과 기본이 꺼짐인 이유는 [5.1](#51-실행-중인-창과-단축-아이콘) 에 있습니다.
