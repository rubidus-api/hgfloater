...
- **파일을 검증하십시오.** 릴리스마다 두 파일의 **SHA-256** 을 적어 두었습니다.
  PowerShell 에서 `Get-FileHash .\hgfloater.exe -Algorithm SHA256` 을 실행하신 결과가
  릴리스 노트의 값과 같다면, 여기서 빌드된 그 파일이 맞습니다.
- **`.zip` 을 받아 보십시오.** 같은 실행 파일을 담은 압축 파일을 함께 올려 두었고,
  브라우저의 다운로드 보호가 이쪽은 덜 까다롭게 보는 편입니다.
...
빌드와 패키징을 이 순서로 **두 번** 돌립니다.

```sh
sh scripts/build-mingw.sh build-mingw   # 경고 없는 빌드, 테스트 컴파일과 호스트 테스트 실행
sh scripts/package-release.sh dist      # exe 준비, 크기를 재서 양쪽 README 에 기록, zip, 체크섬 출력
...
두 번 도는 이유는 About 창이 `README.md` 를 그대로 보여주고, 그 README 가 이제 실행 파일의
**실측 크기**를 적기 때문입니다. 첫 빌드가 숫자를 만들고 두 번째 빌드가 그것을 담습니다.
릴리스 노트에 넣을 체크섬은 마지막 실행이 찍어 준 값입니다.

**두 파일을 항상 함께 올립니다: `hgfloater.exe` 와 `hgfloater-<버전>.zip`.** zip 은
장식이 아니라, 서명 없는 `.exe` 를 브라우저가 막을 때 들어갈 수 있는 두 번째 길이고,
릴리스 노트도 두 파일의 체크섬을 모두 적습니다. `scripts/verify-release.sh` 가 패키징
마지막에 돌면서 둘 중 하나라도 없거나 zip 안에 실행 파일이 없으면 실패시킵니다 — 릴리스가
반쪽인 채로 조용히 나가지 못하게 하는 장치입니다.
