...
> ### 📌 단축 아이콘 등록하는 법
>
> 1. **옵션 메뉴를 엽니다** — `Se` 버튼의 `Opt` 줄, 또는 상태 줄 오른쪽 클릭. 옵션 메뉴가
>    열립니다.
...
- **오른쪽 클릭**하면 옵션 메뉴(`Se` 목록의 `Opt` 줄과 동일)가 열립니다.
...
| 버튼 | 클릭 | 드래그 / 휠 |
| :--- | :--- | :--- |
| **`Siz`** Resize | — | **드래그**로 격자 크기 조절 |
| **`Mov`** Move | **클릭**하면 스스로 비켜남(아래 참고) | **드래그**로 창 이동 |
| **`Ext`** Exit | HGFloater 종료 | — |
| **`Dsk`** Desktop | 모든 창 최소화, 다시 누르면 복원 | — |
| **`Cmd`** Command | [커맨드 박스](#8-커맨드-박스) 열기 | — |
| **`Not`** Note | [노트 목록](#10-노트) 열기 | — |
| **`Clp`** Clipboard | 클립보드 히스토리 열기(`Ctrl + L` 과 동일, 앞에 있을 때 다시 누르면 닫힘) | — |
| **`Dir`** Folders | [폴더 목록](#61-dir-버튼)을 엽니다 — 폴더를 가리키는 단축 아이콘들 | — |
| **`Se`** Settings | [컨트롤 목록](#62-se-버튼)을 엽니다: 볼륨·밝기·투명도·고정·옵션 메뉴 | — |
...
**`Se` 는 버튼 다섯 개였던 것을 하나로 모은 것입니다.** 위에 포인터를 두면 같은 방식의
...
**`Mov` — 비켜나기.** 드래그하지 않고 클릭하면, 방금 있던 영역을 딱 벗어날 만큼만 스스로
...
**`Pin`** 은 `Se` 목록의 한 줄입니다. 고정된 동안에는 마우스를 치워도 태스크박스가 접히지
...
[`Se` 버튼](#62-se-버튼)의 `Opt` 줄, 또는 상태 표시줄 오른쪽 클릭으로 엽니다.
...
| **옵션 메뉴** | `Se` 목록의 `Opt` 줄, 또는 상태 표시줄 오른쪽 클릭 |
| **폴더 열기** | `Dir` 에 포인터를 두고 줄 클릭 |
| **컨트롤 목록** | `Se` 에 포인터 두기 |
| **창 이동** | 빈 공간·상태 표시줄·`Mov` 버튼 왼쪽 드래그 |
| **플로터 이동** | 플로터를 드래그, 또는 `Alt + 드래그` |
| **태스크박스 비켜나기** | `Mov` 버튼 왼쪽 클릭 |
| **격자 크기 변경** | 테두리 드래그 또는 `Siz` 버튼 드래그 |
| **글꼴/아이콘 크기** | `Ctrl` + 휠 |
| **투명도** | `Alt` + 휠, 또는 `Se` 목록의 `Alp` 줄 위에서 휠 |
| **화면 밝기** | `Se` 목록의 `Bri` 줄 위에서 휠 |
| **볼륨 / 음소거** | `Se` 목록의 `Vol` 줄 위에서 휠 / 그 줄 클릭 |
| **태스크박스 고정** | `Se` 목록의 `Pin` 줄 클릭 |
