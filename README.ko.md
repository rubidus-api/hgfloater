...
**v0.15.2** — 빌드 2026-08-15 10:40 KST
...
> ## v0.8.0 에서 있었던 오탐, 이후 빌드에서는 발생하지 않습니다
>
> **v0.8.0** 을 내려받으실 때 일부 브라우저와 백신이 경고를 띄우는 문제가 있었습니다.
> **v0.8.1 이후로는 같은 문제가 재현되지 않습니다.** 경고의 원인이 된 두 기능을 이 빌드에서
> **일시적으로 비활성화**했기 때문입니다.
>
> **왜 의심을 받았는가.** 백신은 프로그램이 특정 Windows 기능을 어떤 목적으로 쓰는지까지
> 판단하지 못합니다. 대신 기능 자체로 분류합니다 — *"이런 종류의 API를 쓰는 프로그램은
> 악성일 확률이 높다"*. 그래서 무해한 기능도 그 API를 썼다는 이유만으로 의심 범주에
> 들어가는데, HGFloater 의 두 기능이 여기에 해당했습니다.
>
> - **최대화 버튼 우클릭 메뉴**는 다른 프로그램의 창에서도 동작해야 하므로 **전역 마우스
>   훅**이 필요했습니다. 이 훅이 한 일은 마우스 버튼과 커서 좌표를 읽어 그 지점이 최대화
>   버튼인지 확인한 것뿐이며, **키보드 훅은 이 프로그램 어디에도 없습니다.** 다만 전역 입력
>   훅은 키로거가 쓰는 것과 같은 계열의 함수라, 정적 분석으로는 둘을 구분하기 어렵습니다.
> - **윈도우 시작 시 실행**은 사용자 계정 범위의 표준 `Run` 키에 `hgfloater` 값 하나를
>   쓰고, 끄시면 그 값을 지웠습니다. 그런데 이 위치는 악성코드가 재부팅 후에도 살아남기
>   위해 쓰는 곳이기도 해서, 값을 쓴다는 사실만으로 의심 점수가 올라갑니다.
>
> 두 기능 모두 해로운 일을 하지 않았고, 다만 그 API가 어울리는 무리 때문에 함께 분류된
> 것입니다. 여기에 **서명이 없고 내려받은 사람도 적다**는 조건이 겹치면 점수를 상쇄할
> 평판이 없어 경고로 이어집니다.
>
> **v0.8.1 이후로** 두 기능이 꺼져 있어, 이 빌드는 훅을 **설치하지 않고** 레지스트리에
> **아무것도 쓰지 않습니다.** 두 메뉴 항목은 `O` 메뉴에 회색으로 남아
> `(off in this build)` 라고 안내합니다. 나머지 — 작업 전환, 탭 목록, 메모, 클립보드,
> 모니터 미리보기, 커맨드 박스, 모니터별 밝기·배율 — 는 전부 그대로 동작합니다. 두 기능은
> 코드 서명 같은 근본적인 해결이 이루어진 뒤에 다시 켜겠습니다.
>
> **받으신 파일은 언제든 확인하실 수 있습니다**: 릴리스마다 두 파일의 **SHA-256** 이 적혀
> 있고, `.exe` 와 함께 **`.zip`** 도 올려 두었으며, 전체 소스가 MIT 라이선스로 공개되어
> 있습니다. 자세한 내용은 [2장](#2-설치와-첫-실행)에 있습니다.
...
- **태스크박스는 자체 배경이 없습니다.** 아이콘 사이는 물론 기능 버튼과 단축 아이콘 뒤로도
  ...
  값을 나타내는 `A`·`B`·`V` 세 버튼만은 색 배경을 그대로 둡니다. 그 색은 꾸밈이 아니라
  현재 값을 읽는 눈금이기 때문입니다. 알아 두실 점 하나: 빈 곳의 클릭은 뒤에 있는 창에
  전달되므로, 창을 옮기실 때는 아이콘 사이가 아니라 `M` 버튼을 끌어 주세요.
...
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v0.15.2)
...
서명되지 않고 내려받은 사람도 적은 프로그램은 경고를 받습니다. 규칙이 설계대로 동작하는
것이므로, "믿어 주십시오"라고 말씀드리는 대신 직접 확인하실 수 있는 방법을 안내드립니다.
...
휴리스틱 검사가 주목하는 Win32 함수는 보통 다음 세 가지입니다. 각각 눈에 보이는 기능
하나를 위해 쓰이며, 숨겨 둔 용도는 없습니다. **v0.8.1 에서는 앞의 두 가지가 꺼져 있어서**
— 이 문서 맨 위의 안내를 참고해 주세요 — 배포본은 훅을 설치하지도, 레지스트리에 무언가를
쓰지도 않습니다.

| 검사기가 보는 것 | 실제 용도 |
| :--- | :--- |
| 저수준 **마우스** 훅 | [최대화 버튼 메뉴](#6-툴바)입니다. 마우스 버튼과 커서 좌표만 읽으며, **이 프로그램에 키보드 훅은 없습니다**. `caption_menu=0` 으로 끄시면 훅 자체가 설치되지 않습니다. |
| **레지스트리** 기록 | **윈도우 시작 시 실행** 토글 전용입니다. 일반적인 사용자별 `Run` 키에 `hgfloater` 값 하나를 쓰고, 끄시면 그 값을 지웁니다. |
| **`OpenProcess`** | 전환 목록에 표시할 창의 프로그램 이름과 아이콘을 읽기 위한 것입니다. 조회 전용 권한(`PROCESS_QUERY_LIMITED_INFORMATION`)으로만 열며 다른 프로세스의 메모리에는 접근하지 않습니다. |

이 바이너리가 명백히 할 수 없는 일도 있습니다. **네트워크 접속**입니다. 소켓·HTTP·
다운로드 관련 함수를 하나도 임포트하지 않으므로, 외부로 무언가를 보내거나 받아 올
수단이 아예 없습니다.
...
`O` 메뉴의 **Show Tabs as Task Icons** 또는 `config.ini` 의 `[taskbox] show_tabs=1` 로 켭니다.
