...
API만으로 작성되어 외부 의존성이 전혀 없습니다. 프로그램 전체가 **약 390 KB짜리 실행 파일
...
| `list note` | `l n` | 모든 노트를, `note` 명령이 받는 번호와 함께 표시 |
| `list sensors` | `l t` | 찾아낸 온도 센서 전부와, `TMP`·`GPU`가 무엇을 보여주는지 |
...
| `search note 단어` | `s n 단어` | 제목이나 **본문**에 `단어`가 든 노트들을, `list note` 번호 그대로 표시 |
