...
- **왼쪽 클릭** — 태스크박스를 토글합니다. 그 자리에서 열리며, 태스크박스가 떠 있는 동안
  플로터는 스스로 숨습니다. 포인터가 플로터 위를 지나가기만 해서는 아무 일도 없습니다.
- **왼쪽 드래그** — **`F` 조정 모드**에서 어느 모니터로든 옮깁니다. 그 모드가 아니면
  플로터를 눌렀다 떼는 동작이 클릭으로 읽혀 태스크박스가 토글되므로, 먼저 `F`를
  누르십시오.
- **Alt + 휠** — 투명도.
...
**`F` — 플로터 조정.** 대시보드를 접으므로 `Ctrl + 휠`(크기)과 `Alt + 휠`(투명도)로
플로터를 조정할 수 있고, **드래그로 원하는 자리에 옮길 수도** 있습니다. 옮기는 것이 이
모드에서만 되는 이유는, 다른 곳에서는 플로터를 눌렀다 떼면 클릭으로 읽혀 태스크박스가
열리기 때문입니다. 드래그하지 않고 클릭하면 돌아옵니다.
