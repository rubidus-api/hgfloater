...
**v0.17.0** — 빌드 2026-08-23 17:00 KST
...
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v0.17.0)
...
| `Tab` | 박스 안으로 들어갑니다. 테두리가 생기고, 이후 키는 박스의 것이 됩니다 |
| `위`/`아래`, `Home`/`End` | (박스 안에서) 선택을 옮깁니다 |
| `a`~`z`, `A`~`Z` | 열 번째 줄부터의 라벨입니다. 박스 안에서만 동작합니다 — 밖에서는 그 글자들이 그리드를 움직이기 때문입니다 |
| `Enter` / `Space`, 또는 클릭 | 선택한 탭으로 전환합니다 |
| 오른쪽 클릭 | 그 탭을 닫습니다. 목록은 그대로 남습니다 |
| `Esc` | 박스만 닫습니다. 키보드는 태스크박스에 그대로 있습니다 |
...
단어를 버튼 크기에 맞춰 표시하며, 그중 둘은 무언가를 바로 하는 대신 **목록을 엽니다.**
...
**`Set` 은 버튼 다섯 개였던 것을 하나로 모은 것입니다.** 위에 포인터를 두면 같은 방식의
목록이 열리고, 한 줄에 하나씩 들어 있습니다.

| 줄 | 하는 일 |
| :--- | :--- |
| **`Vol`** | 시스템 볼륨과 현재 값. 줄 위에서 **휠**을 돌리면 5% 단위로 바뀌고, **클릭**하면 음소거를 토글합니다 |
| **`Bri`** | 화면 밝기와 현재 값. **휠**로 5% 단위 조절 |
| **`Alp`** | 태스크박스 투명도와 현재 값. **휠**로 조절 |
| **`Pin`** | 태스크박스 고정. 고정된 동안에는 마우스를 치워도 접히지 않습니다. **클릭**으로 토글 |
| **`Opt`** | [옵션 메뉴](#7-옵션-메뉴)를 엽니다 |
...
- **Options** — 켜고 끄는 항목은 모두 이 하위 메뉴에 모여 있습니다. 켜져 있으면 체크가
  ...
  - **Open the Taskbox on Hover** — 기본은 꺼짐입니다. 켜면 v0.13.0 이전처럼 포인터가
    **시계의 분** — 콜론 뒤의 두 자리 — 위에 머무는 것만으로 대시보드가 열리고, 꺼져
    있으면 클릭으로 엽니다. 그 두 자리만 반응합니다. 플로터의 나머지는 손이 다른 곳으로
    가다 스치는 자리이고, 분은 시계의 끝이라 포인터가 위젯을 가로지르지 않고 바깥에서
    곧장 닿기 때문입니다. 어느 쪽이든 **클릭은 플로터 어디서나** 태스크박스를 엽니다.
    [4장](#4-플로터)을 보세요.
  - **Outline the Window Under the Pointer** — 기본은 켜짐입니다. 작업 아이콘을 가리키면
    그 아이콘이 가리키는 창의 자리에 테두리를 그립니다. 전환하지 않고도 "저게 어느
    창인지" 알 수 있습니다.
  - **Show a Window's Tabs on Hover** — 기본은 켜짐입니다. 탭이 있는 창의 아이콘을
    가리키면 그 옆에 탭 목록이 열립니다. 탭을 읽어 오는 것 자체는
    **Show Tabs as Task Icons** 가 하는 일이라, 그 항목도 켜져 있어야 합니다.
  ...
  - **Stat Bars on the Floater** — 시계 뒤에 깔리는 배터리·CPU·메모리·온도 막대입니다.
    v0.14.0 이전에는 `config.ini` 에만 있고 어디에서도 켜고 끌 수 없었습니다.
