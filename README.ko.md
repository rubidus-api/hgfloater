...
**v0.17.1** — 빌드 2026-08-23 18:20 KST
...
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v0.17.1)
...
| 버튼 | 클릭 | 드래그 / 휠 |
| :--- | :--- | :--- |
| **`ReS`** Resize | — | **드래그**로 격자 크기 조절 |
| **`Mv`** Move | **클릭**하면 스스로 비켜남(아래 참고) | **드래그**로 창 이동 |
| **`Ext`** Exit | HGFloater 종료 | — |
| **`Dsk`** Desktop | 모든 창 최소화, 다시 누르면 복원 | — |
| **`Com`** Command | [커맨드 박스](#8-커맨드-박스) 열기 | — |
| **`Nt`** Note | [노트 목록](#10-노트) 열기 | — |
| **`Clp`** Clipboard | 클립보드 히스토리 열기(`Ctrl + L` 과 동일, 앞에 있을 때 다시 누르면 닫힘) | — |
| **`Dir`** Folders | [폴더 목록](#61-dir-버튼)을 엽니다 — 폴더를 가리키는 단축 아이콘들 | — |
| **`Set`** Settings | [컨트롤 목록](#62-set-버튼)을 엽니다: 볼륨·밝기·투명도·고정·옵션 메뉴 | — |

### 6.1 Dir 버튼

**`Dir` 위에 포인터를 두거나 클릭하면** 버튼 옆에 폴더 목록이 열립니다 — 화면에 맞는
...
**`Set` 은 "설정하는 것" 전부입니다.** 위에 포인터를 두면 같은 방식의 목록이 열립니다.
...
않습니다. `Ext`, `Esc`, 전역 단축키, 플로터 클릭 같은 명시적인 닫기는 그대로 동작합니다.
다시 클릭하면 해제됩니다.


### 클립보드 히스토리

툴바의 **`Clp`** 버튼, 또는 플로터·태스크박스에서 **`Ctrl + L`** 을 누르시면 복사해 온
...
| **격자 크기 변경** | 테두리 드래그 또는 `ReS` 버튼 드래그 |
| **글꼴/아이콘 크기** | `Ctrl` + 휠 |
