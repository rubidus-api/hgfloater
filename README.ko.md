...
> ## ⚠ v0.8.1 은 두 기능을 일부러 꺼 둔 빌드입니다
>
> **Start with Windows**(윈도우 시작 시 실행)와 **Menu on Maximize Button**(최대화 버튼
> 우클릭 메뉴)이 이 빌드에서는 **일시적으로 비활성화**되어 있습니다. 두 항목은 `O` 메뉴에
> 그대로 있으며, 회색으로 표시되고 그 사유를 함께 보여 드립니다.
>
> 두 기능 모두 문제가 될 일을 하지는 않았습니다. 다만 전역 마우스 훅과 `Run` 키 기록은
> 휴리스틱 검사가 가장 무겁게 보는 두 가지이고, 서명이 없고 다운로드 수가 수십 회뿐인
> 파일에는 그에 맞설 평판이 없습니다. 그래서 배포본에서 빼 보기로 했습니다 — **빼고도
> 경고가 계속된다면 원인은 애초에 코드가 아니었다는 뜻**이고, 그것을 아는 것만으로도
> 의미가 있습니다.
>
> 정리되는 대로 되돌리겠습니다. 나머지 기능은 그대로 동작합니다.
>
> **브라우저나 백신이 경고를 띄운다면**, 그것은 무언가를 찾아낸 것이 아니라 평판에 대한
> 경고입니다. 이 파일은 코드 서명이 되어 있지 않고 릴리스마다 아무도 아직 받지 않은 새
> 파일이라, 브라우저의 다운로드 보호와 머신러닝 기반 백신 규칙이 반응하는 조건을 그대로
> 갖추고 있습니다. 대신 직접 확인하실 수 있는 것들을 준비해 두었습니다. 전체 소스가 MIT
> 라이선스로 여기 공개되어 있고 동일하게 재현되는 빌드이며, 릴리스 노트에 파일마다
> **SHA-256** 이 적혀 있어 받으신 파일을 검증하실 수 있고, 이 바이너리에는 **네트워크
> 관련 함수가 하나도 들어 있지 않습니다** — 소켓도, HTTP도, 다운로드 기능도 없습니다.
> 같은 실행 파일을 담은 **`.zip`** 도 함께 올려 두었는데, 브라우저에 따라 이쪽을 덜
> 까다롭게 봅니다. 휴리스틱이 주목하는 Win32 함수 세 가지와 각각의 용도는
> [2장](#2-설치와-첫-실행)에 적어 두었습니다.
