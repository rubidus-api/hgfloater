...
**v0.8.1** — 빌드 2026-08-09 22:40 KST
...
> ## v0.8.0 에서 있었던 오탐, v0.8.1 에서는 발생하지 않습니다
>
> **v0.8.0** 을 내려받으실 때 일부 브라우저와 백신이 경고를 띄우는 문제가 있었습니다.
> **v0.8.1 에서는 같은 문제가 재현되지 않습니다.** 경고의 원인이 된 두 기능을 이 빌드에서
> **일시적으로 비활성화**했기 때문입니다.
>
...
> 평판이 없어 경고로 이어집니다.
>
> **v0.8.1 에서는** 두 기능이 꺼져 있어, 이 빌드는 훅을 **설치하지 않고** 레지스트리에
> **아무것도 쓰지 않습니다.** 두 메뉴 항목은 `O` 메뉴에 회색으로 남아
> `(temporarily disabled)` 라고 안내합니다. 나머지 — 작업 전환, 탭 목록, 메모, 클립보드,
...
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v0.8.1)
