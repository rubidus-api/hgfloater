...
### English
1. **Download**: Get the latest `hgfloater.exe` from the [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.05.31) page.
...
### 한국어
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.05.31) 페이지에서 최신 `hgfloater.exe` 파일을 다운로드하세요.
...
- **Hover**: Simply move the mouse over the floater to instantly summon the taskbox in place (the floater hides itself). The taskbox auto-returns to the floater shortly after the cursor leaves it (a ~1 second grace period prevents accidental hide if the cursor briefly re-enters).
...
## 4. 마우스 조작 안내
### 플로팅 위젯 (floater)
- **마우스 호버(올리기)**: 위젯 위에 마우스를 올리기만 하면 그 자리에 taskbox가 즉시 나타납니다(플로터는 스스로 숨김). 커서가 taskbox를 벗어나면 잠시 후 자동으로 다시 플로터로 돌아옵니다(약 1초의 유예 시간이 있어, 커서가 잠깐 벗어났다 다시 들어오면 숨김이 취소됩니다).
