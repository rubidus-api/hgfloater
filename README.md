...
- **Status Line**: A third line under the date shows battery percentage (`+` marks charging; hidden on systems without a battery), CPU usage, and memory usage, refreshed once per second.
...
- **상태 표시줄**: 날짜 아래 세 번째 줄에 배터리 잔량(충전 중이면 `+`, 배터리 없는 시스템에서는 숨김), CPU 점유율, 메모리 사용률이 1초 간격으로 표시됩니다.
