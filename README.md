...
- **Toolbar Rich Interaction Buttons**:
  - **Move Handle**: **Left Drag** to move the taskbox window. **Left Click** (without dragging) moves it aside on its own: it looks north, then west, then south, then east, and sends the floater and taskbox to the first work-area edge where they no longer cover the spot they were just sitting on. If no direction is clear, they stay put.
...
| **Move Aside** | **Left-click** the Move-handle toolbar button (north, west, south, east) |
...
- **툴바 버튼 다채로운 조작**:
  - **이동 핸들 (Move Handle)**: **왼쪽 드래그**로 태스크 박스 창을 화면 내에서 자유롭게 이동합니다. **왼쪽 클릭**(드래그하지 않고)하면 스스로 비켜납니다: 북쪽, 서쪽, 남쪽, 동쪽 순서로 살펴서 지금 있던 영역과 전혀 겹치지 않는 첫 번째 작업 영역 가장자리로 플로터와 태스크 박스를 함께 보냅니다. 갈 수 있는 방향이 없으면 그대로 있습니다.
