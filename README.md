...
### English
1. **Download**: Get the latest `hgfloater.exe` from the [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.07.19b) page.
...
### 한국어
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.07.19b) 페이지에서 최신 `hgfloater.exe` 파일을 다운로드하세요.
...
- **Toolbar Rich Interaction Buttons**:
  - **Move Handle**: **Left Drag** to move the taskbox window. **Left Click** (without dragging) nudges it aside on its own: it looks north, then west, then south, then east, and takes the first direction with room, moving just far enough to stop covering the spot it was sitting on. If no direction has room, it stays put.
...
| **Move Aside** | **Left-click** the Move-handle toolbar button (smallest step clear of the current spot: north, west, south, east) |
...
- **툴바 버튼 다채로운 조작**:
  - **이동 핸들 (Move Handle)**: **왼쪽 드래그**로 태스크 박스 창을 화면 내에서 자유롭게 이동합니다. **왼쪽 클릭**(드래그하지 않고)하면 스스로 살짝 비켜납니다: 북쪽, 서쪽, 남쪽, 동쪽 순서로 살펴서 자리가 있는 첫 번째 방향으로, 방금 있던 영역을 딱 벗어날 만큼만 움직입니다. 갈 수 있는 방향이 없으면 그대로 있습니다.
