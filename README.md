...
### English
1. **Download**: Get the latest `hgfloater.exe` from the [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.07.10) page.
...
### 한국어
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.07.10) 페이지에서 최신 `hgfloater.exe` 파일을 다운로드하세요.
...
- **Hover**: Simply move the mouse over the floater to instantly summon the taskbox in place (the floater hides itself). The taskbox auto-returns to the floater shortly after the cursor leaves it (a ~0.5 second grace period prevents accidental hide if the cursor briefly re-enters).
...
- **Status Bars**: A compact panel on the floater's left shows CPU, MEM, and BAT horizontal bar graphs (red/blue/green, full width = 100%) with tiny labels, refreshed once per second; the battery row hides on systems without one and `+` on the label marks charging.

> Note: The floater no longer has its own right-click menu. The main settings/context menu now lives on the taskbox **`P` (PopupMenu)** toolbar button (see below).
...
- **Toolbar Rich Interaction Buttons**:
  - **Move Handle**: **Left Drag** to move the taskbox window.
  ...
  - **`P` Button (PopupMenu)**: **Left Click** opens the main context menu:
...
| **Reset Layout & Settings** | `Ctrl` + `R` / `0` or `F5` to restore default position, opacity, and sizes (Floater/Taskbox), or select "Reset Settings" from the `P` (Popup Menu) |
...
| **Focus Command Input** | `Ctrl + Space` inside the Command Box |
| **Exit Application** | `Ctrl` + `Q` / `X` or `Alt` + `F4`, or select "Exit" from the `P` (Popup Menu) |
...
## 4. 마우스 조작 안내
### 플로팅 위젯 (floater)
- **마우스 호버(올리기)**: 위젯 위에 마우스를 올리기만 하면 그 자리에 taskbox가 즉시 나타납니다(플로터는 스스로 숨김). 커서가 taskbox를 벗어나면 잠시 후 자동으로 다시 플로터로 돌아옵니다(약 0.5초의 유예 시간이 있어, 커서가 잠깐 벗어났다 다시 들어오면 숨김이 취소됩니다).
- **왼쪽 클릭**: taskbox 보이기/숨기기 토글.
- **왼쪽 드래그**: 위젯 창을 화면 원하는 곳으로 이동.
- **Alt + 마우스 휠**: 위젯의 투명도 조절.
- **상태 막대**: 플로터 왼쪽의 컴팩트한 패널에 CPU/MEM/BAT 가로 막대 그래프(빨강/파랑/초록, 전체 폭 = 100%)가 아주 작은 라벨과 함께 1초 간격으로 표시됩니다. 배터리 없는 시스템에서는 BAT 줄이 숨겨지고, 충전 중에는 라벨에 `+`가 붙습니다.
...
| **레이아웃 및 크기 초기화** | `Ctrl` + `R` / `0` 또는 `F5` (기본 좌표, 투명도, 크기로 초기 설정 복구), 혹은 `P`(Popup Menu) 버튼 메뉴의 "Reset Settings" 선택 |
...
| **애플리케이션 종료** | `Ctrl` + `Q` / `X` 또는 `Alt` + `F4`, 또는 `P`(Popup Menu) 버튼 메뉴에서 "Exit" 선택 |
