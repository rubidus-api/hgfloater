# <img src="hgfloater.ico" width="32" height="32" alt="icon" style="vertical-align: middle; margin-right: 8px;">hgfloater
...
### English
1. **Download**: Get the latest `hgfloater.exe` from the [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.04.63) page.
...
### 한국어
1. **다운로드**: [Releases](https://github.com/rubidus-api/hgfloater/releases/tag/v26.04.30) 페이지에서 최신 `hgfloater.exe` 파일을 다운로드하세요.
...
- **Right Click (Top Console)**: Opens a context menu to "Copy all logs", capturing the entire rolling log buffer to the clipboard.
- **Ctrl + Mouse Wheel**: Adjust the size of icons and the window natively while keeping rows static. When focusing specifically over the Top Edit control, solely scales the target font text internally.
- **Alt + Mouse Wheel**: Adjust transparency of the taskbox.
- **Border Drag**: Resize the taskbox from any edge or corner, automatically expanding/contracting grid cells.
...
- **Movement**: `Alt` + `Arrow Keys` / `HJKL` / `WASD`
- **Grid Scaling**: `Ctrl` + `Up/Down` to adjust row capacity dynamically in the taskbox. Adds or removes column limits directly. `Ctrl` + `Left/Right` adds or removes 1 grid column array. Note: Effectively expands/shrinks grid capacity cleanly.
- **Font Scaling**: `Ctrl` + `+/-` scales text fonts individually natively.
...
- **빈 공간 또는 상단 로그 영역 왼쪽 드래그**: 창 이동 수행.
- **툴바 'R' 버튼 왼쪽 드래그**: taskbox 창 크기를 마우스 움직임에 따라 실시간 조절.
- **툴바 'X' 버튼**: taskbox를 숨기고, 바로가기 폴더로부터 **아이콘 정보를 자동으로 다시 읽어옵니다**.
...
### 창 조작 (포커스된 창: floater 또는 taskbox)
- **위치 이동**: `Alt` + `방향키` / `HJKL` / `WASD`
- **그리드 재배열**: `Ctrl` + `방향키`를 이용하여 실제 창(프레임) 사이즈 조절이 아니라 내부 가로 열 갯수 혹은 세로 행 수를 동적으로 증감. 좌우는 1칸씩, 상하는 다채롭게 가감하여 태스크박스 크기가 셀에 딱 맞게 정렬됩니다.
- **폰트 변경**: `Ctrl` + `+/-` 를 통해 로그 콘솔 및 플로터 텍스트의 크기만 독립적으로 조절.
- **투명도 조절**: `Alt` + `+` (진하게) / `-` (투명하게)
